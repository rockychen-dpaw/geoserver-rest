...
class WMSServiceMixin(object):
    ...
    def get_map(self,workspace,layername,bbox,version="1.1.0",srs="EPSG:4326",width=1024,height=1024,format="image/jpeg",style="",outputfile=None):
        """
        outputfile: a temporary file will be created if outputfile is None, the client has the responsibility to delete the outputfile, or will be deleted when the program exits.
        If succeed, save the image to outputfile
        """
        if outputfile:
            output = open(outputfile,'w')
        else:
            output = tempfile.NamedTemporaryFile(prefix="gs_healthcheck",suffix=format.rsplit("/",1)[1] if "/" in format else format,delete = False)
        ...
        if res.headers.get("content-type") != format:
            if res.headers.get("content-type","").startswith("text/"):
                ...
                raise GetMapFailed("Failed to get the map of layer({}:{}).Expect '{}', but got '{}'".format(workspace,layername,format,res.header.get("content-type","")))
        if outputfile:
            output = open(outputfile,'wb')
        else:
            output = tempfile.NamedTemporaryFile(
                mode='wb',
                prefix="gs_healthcheck",
                suffix=format.rsplit("/",1)[1] if "/" in format else format,
                delete = True, 
