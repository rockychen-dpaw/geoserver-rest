...
class GWCManage(object):
    def __init__(self,geoserver_name,geoserver_url,geoserver_user,geoserver_password,ssl_verify,gwc_tiles_dir,gwc_disk_size,gwc_disk_shared,requestheaders=None):
        self.geoserver_name = geoserver_name
        self.gwc_tiles_dir = gwc_tiles_dir
        self.gwc_disk_size = gwc_disk_size
        self.gwc_disk_shared = gwc_disk_shared
    ...
    def get_gwc_disk_usage_status(self):
        """
        unit is K
        Return disksize,used size if gwc_tiles_dir is a mounted volume; otherwise return None
        """
        if self.gwc_disk_shared:
            ...
            if result.returncode != 0:
                ...
                logger.debug("Failed to find the disk usage via command 'df' because the gwc tile folder '{}' is not a mounted volume ".format(self.gwc_tiles_dir))
                return None
    ...
    def clean_layer_cache(self,layer,emergency=False):
        """
        Return True if take the real clean action; otherwise return False
        ...
        if "cache_starttime" in layer:
            try:
                cache_starttime = datetime.strptime(layer["cache_starttime"],"%Y-%m-%d %H:%M:%S.%f")
            except:
                pass

        try:
            starttime = datetime.now()
            layer["clean_starttime"] = starttime.strftime("%Y-%m-%d %H:%M:%S.%f")
            ...
            if os.path.exists(layer_tile_dir):
                ...
                layer["cache_starttime"] = (now - timedelta(seconds = minutes * 60)).strftime("%Y-%m-%d %H:%M:%S.%f")
                if result.returncode != 0:
                    raise Exception(result.stderr)
            elif expireCache > 0 :
                layer["cache_starttime"] = now.strftime("%Y-%m-%d %H:%M:%S.%f")
            else:
                if "cache_starttime" in layer:
            ...
            #find the cache size of the layer no matter clean status
            if os.path.exists(layer_tile_dir):
            ...
            layer["clean_endtime"] = endtime.strftime("%Y-%m-%d %H:%M:%S.%f")
    ...
    def get_diskusage(self,diskusagestatus):
        ...
        if diskusage["disksize"] <= 0:
            ...
            if diskusage["cachesize"] / 1048576 >= 5:
                #more than 5G
                diskusage["cachesize"] = "{:,.2f}G".format(diskusage["cachesize"] / 1048576)
            elif diskusage["cachesize"] / 1024 >= 5:
                #more than 5M
            ...
            if diskusage["disksize"] / 1048576 > 5:
                #more than 5G
                diskusage["disksize"] = "{:,.2f}G".format(diskusage["disksize"] / 1048676)
            elif diskusage["disksize"] / 1024 > 5:
        ...
        diskusage["checktime"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")

        return diskusage

    def post_clean(self):
        #get total cache size and populate the clean status
        gwccleanstatus = {}
        cachesize = 0
        ...
        diskusage = self.get_diskusage(self.get_gwc_disk_usage_status())
    ...
    def clean_gwc_cache(self,gwc_cleantime=0):
        ...
        if lastcleanbatchid:
            ...
            for layer in self._layers:
                if layer.get("cleanbatchid") == lastcleanbatchid and layer.get("clean_succeed",False):
                    #already cleaned before
                    continue
                layer["cleanbatchid"] = lastcleanbatchid
                self.clean_layer_cache(layer)
                if gwc_cleantime > 0 and (datetime.now() - started).total_seconds() > gwc_cleantime:
                    #done
                    break

        if gwc_cleantime <= 0 or (datetime.now() - started).total_seconds() < gwc_cleantime:
            #still have time,continue clean other layers
            cleanbatchid = started.strftime("%Y-%m-%d %H:%M:%S.%f")
            for layer in self._layers:
    ...
    def check_gwc_cache(self,threadhold=0.9):
        ...
        while True:
            diskusagestatus = self.get_gwc_disk_usage_status()
            if not diskusagestatus:
                logger.warning("GWC cache checking function is disable for non-mounted gwc volume.")
                break
